...
def remap_obs(env_obs: dict, device: torch.device) -> dict:
    ...
    # obj_to_eef: use env key if available, otherwise zeros (policy still runs)
    obj_to_eef_np = env_obs.get(
        "object_to_robot0_eef_pos",
        env_obs.get("obj_to_eef", np.zeros(3, dtype=np.float32))
    )
    ...
    return {
        ...
        "obj_to_eef":      to_tensor(obj_to_eef_np),
    }
...
def main():
    ...
    parser.add_argument("--head_type",   type=str, default="gmm",
                        choices=["deterministic", "gmm"],
                        help="Action head type: 'deterministic' or 'gmm' (default: gmm)")
    ...
    # ── Load policy ──────────────────────────────────────────────────── #
    policy = BCRNNPolicy(use_gmm=(args.head_type == "gmm")).to(device)
